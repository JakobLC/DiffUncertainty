...
class ProbabilisticUnet(nn.Module):
    ...
    def elbo(self, segm, analytic_kl=True, reconstruct_posterior_mean=False):
        ...
        return -(self.reconstruction_loss + self.beta * self.kl), self.reconstruction_loss, self.kl
    ...
    def get_pred(self, patch, sigmoid=True):
        ...
        mask_pred = self.sample(testing=False)
        return s(mask_pred)
...
from jlc_utils_for_lidc.jlc_utils import iou_torch, get_dataloaders
from probabilistic_unet import ProbabilisticUnet
from utils import l2_regularisation
import tqdm
import argparse
from sample import eval
from jlc import num_of_params

parser = argparse.ArgumentParser(description='Train a probabilistic U-Net on LIDC data')
parser.add_argument('--save_filename', type=str, default='./saves/model_save.pth')
parser.add_argument('--batch_size', type=int, default=8)
parser.add_argument('--old_split', type=bool, default=False)
parser.add_argument('--lr', type=float, default=1e-4)
parser.add_argument('--latent_dim', type=int, default=6)
parser.add_argument('--epochs', type=int, default=20)
parser.add_argument('--beta', type=float, default=10.0)
parser.add_argument('--gradient_clipping', type=float, default=1.0)
parser.add_argument('--filter_idx', type=int, default=0)
args = parser.parse_args()
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

train_loader,_,_ = get_dataloaders(batch_size=args.batch_size,verbose=True)
train_loader_eval,vali_loader,_ = get_dataloaders(batch_size=args.batch_size,verbose=False,augment="none")

idx_to_filter = {0: [32 ,64 ,128,192],      # 5m params
                1:  [32 ,64 ,96 ,128,256],  # 8m params
                2:  [64 ,128,196,256,512],  # 20m params
                } 
net = ProbabilisticUnet(input_channels=1, 
                        num_classes=1, 
                        num_filters=idx_to_filter[args.filter_idx],
...
losses = {"elbo": [], "recon": [], "kl": [], "l2reg": [], "iou": [], "loss": []}
vali_metrics = {}
train_metrics = {}
step = 1
tqdm_steps = tqdm.tqdm(total=total_steps, desc=f"Epoch [{0}/{args.epochs}], Step [{0}/{total_steps}], Loss: {float("nan"):10.4f}")
for epoch in range(args.epochs):
    for patch, mask, _ in train_loader:
        patch = patch.to(device)

        #get random mask
        mask = mask[:,np.random.randint(0,mask.shape[1]),:,:]

        mask = mask.to(device)
        mask = torch.unsqueeze(mask,1)
        net.forward(patch, mask, training=True)
        mask_pred = net.sample() 
        
        elbo,recon_loss,kl_loss = net.elbo(mask)
        ...
        optimizer.step()
        iou_metric = np.mean([iou_torch(mask[i] > 0.5, mask_pred[i] > 0) for i in range(mask.shape[0])])

        #add losses to dictionary
        losses["elbo"].append(elbo.item())
        losses["recon"].append(recon_loss.item())
        losses["kl"].append(kl_loss.item())
        losses["l2reg"].append(reg_loss.item())
        losses["iou"].append(iou_metric)
        losses["loss"].append(loss.item())

        #update tqdm
        tqdm_steps.update(1)
        tqdm_steps.set_description(f"Epoch [{epoch}/{args.epochs}], Step [{step}/{total_steps}], Loss: {loss.item():10.4f}")
        step += 1

    metrics, mean_metrics = eval(model=net, eval_loader=vali_loader, num_samples=1.0, device=device)   
    vali_metrics[step] = mean_metrics
    metrics, mean_metrics = eval(model=net, eval_loader=train_loader_eval, num_samples=0.1, device=device)
    train_metrics[step] = mean_metrics
    torch.save({"net_state_dict": net.state_dict(),
                        "losses": losses,
                        "vali_metrics": vali_metrics,
                        "train_metrics": train_metrics,
                        "step": step,
                        "args": args}, args.save_filename)